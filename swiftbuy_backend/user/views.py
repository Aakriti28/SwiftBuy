...
def signup(request):
	...
	form = RegisterUserForm({'name': params['name'], 'email': params['email'], 'phone': params['phone'], 'address': params['address'], 'shipping_address': params['shipping_address'], 'referral_token': params['referral_token'], 'giver_token': user_info['referralToken'], 'password1': params['password'], 'password2': user_info['cpassword'], 'role': params['role'], 'wallet_amount': 0})
	...
	elif params['password'] != user_info['cpassword'] :
		return JsonResponse({'status': 'failure', 'results': form.errors}, status=432)
	elif not form.referral_token_is_valid():
		return JsonResponse({'status': 'failure', 'results': form.errors}, status=487)
	else :
		return JsonResponse({'status': 'failure', 'results': form.errors}, status=415)
...
def mylogin(request):
	...
	if user is not None:
		...
		if Users.objects.filter(email=user_info['email'], password=user_info['password']).exists():
			user = Users.objects.filter(email=user_info['email'], password=user_info['password'])[0]
			login(request, user)
			return JsonResponse({'status': 'success'}, status=HTTPStatus.OK)
